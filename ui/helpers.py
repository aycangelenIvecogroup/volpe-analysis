...
HELP = {
    ...
    "coverage_pct": "Sales execution indicator: Actual Net Sales (TN) divided by Budget 2026 Net Sales.",
    "units": "Number of units sold in the selected period.",
    "tn": "Total Net Sales (TN) value for the selected period.",

    # --- DERIVED / COMPARISON ---
    "agm_gap_b26": "Difference between actual AG%M and Budget 2026 AGM%.",
    "agm_yoy": "Year-over-year change in AGM% versus 2025.",
    "sales_gap": "Remaining gap to reach 100%, budget coverage.",

    # --- RISK ---
    "risk_level": "Risk classification based on Coverage % thresholds (CRITICAL / WARNING / OK).",
...
def explain_overview(df, month: str, critical_below: float):
    ...
    if avg_coverage < 100:
        explanations.append(
            "Overall sales execution is **below plan**, indicating a **volume-related risk**."
        )

    if avg_agm < 0:
        explanations.append(
            "Average margin is negative, pointing to **structural profitability issues**."
        )

    if critical_count > 0:
        explanations.append(
            f"There are **{critical_count} customers in CRITICAL status** requiring immediate attention."
        )

    if not explanations:
        explanations.append(
            "Sales execution and profitability are broadly **in line with targets**."
        )
